...
class Engine {
  ...
  convertToIssue(message, path, contents) {
    return {
      type: 'issue',
      categories: ['Style'],
      remediation_points: 50000,  // eslint-disable-line camelcase
  ...
  resolvePackageConfiguration(pkg) {
    if (pkg && pkg['remarkConfig']) {
      var rules = pkg['remarkConfig']['plugins'] && pkg['remarkConfig']['plugins']['lint'];
      return Object.assign(this.resolvePresets(pkg['remarkConfig']), rules);
    }
  }
